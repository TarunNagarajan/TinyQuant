...
def extract_answer(text):
    # Base models usually follow the pattern from the prompt, so we look for "Answer:"
...
def main():
    ...
    print(f"Running StableLM-2-2B Benchmark | Mode: {args.mode}")
    ...
    for idx in tqdm(range(len(dataset))):
        ...
        # Construct Prompt for StableLM (Raw completion format)
    ...
    print(f"StableLM-2-2B {args.mode} Result: {accuracy:.2%} | Size: {model_size:.2f}MB | VRAM: {peak_vram:.2f}GB")
