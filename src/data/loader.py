...
def get_calibration_data(dataset_name, tokenizer, n_samples = None):
    ...
    if dataset_name == "gsm8k":
        ds = load_dataset("gsm8k", "main", split = "train", download_mode="reuse_cache_if_exists", trust_remote_code=True)
        ds = ds.shuffle(seed = 42).select(range(n_samples))
        data = [format_gsm8k_chat(row, tokenizer) for row in ds]

    elif dataset_name == "wikitext":
        ds = load_dataset("wikitext", "wikitext-2-raw-v1", split = "train", download_mode="reuse_cache_if_exists", trust_remote_code=True)
